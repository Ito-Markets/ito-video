...
- Sidechain-style ducking: music dips during voiceover via sidechaincompress
- Uses asplit to properly fan out voiceover stream
...
def main():
    ...
    if probe.returncode != 0 or not probe.stdout.strip():
        print(f"WARNING: ffprobe failed (rc={probe.returncode}), using 102s fallback")
        vid_dur = 102.0
    else:
        vid_dur = float(probe.stdout.strip())

    # v2: Sidechain ducking mix
    # - Voiceover split via asplit: one copy for sidechain detection, one for final mix
    # - Music bed at -18dB normally, ducked further when VO is active
    # - Music fades in over 4s, fades out starting 8s before end
    fade_out_start = max(0, vid_dur - 8)
    fc = (
        f"[0:v]copy[v];"
        f"[1:a]volume=1.0,apad=whole_dur={vid_dur},asplit[vo][vo_sc];"
        f"[2:a]volume=-18dB,"
